...
sumC (x1, y1, z1, q1) (x2, y2, z2, q2) = (x1 + x2, y1 + y2, z1 + z2, q1 + q2)
...
parse xs = M.fromList $ getActive' list
  where
    ...
    generate n (x:xs) = first ++ generate (n+1) xs
      where first = map (\(x, y) -> ((n, x, 0, 0), readCell y )) $ zip [0..] x
...
getNeighbours field coo = [sumC coo (x, y, z, q) | x <- step, y <- step, z <- step, q <- step, (x, y, z, q) /= (0,0,0,0)]
